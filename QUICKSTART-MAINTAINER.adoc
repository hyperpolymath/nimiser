...
// Replace nimiser, nimiser, {{DEPS}} with actuals
...
| `$PREFIX/share/nimiser/`
| Data files, assets

| `$PREFIX/share/doc/nimiser/`
| Documentation
...
Default config location: `$XDG_CONFIG_HOME/nimiser/config.toml`

Fallback: `$HOME/.config/nimiser/config.toml`
...
Or via OPSM: `opsm update nimiser`
...
[source,bash]
----
just install --prefix=/opt/nimiser-instance1 --config=/etc/nimiser/instance1.toml
just install --prefix=/opt/nimiser-instance2 --config=/etc/nimiser/instance2.toml
----
